...
Для локального Neo4j Browser используются значения из `.env`:

- логин: `neo4j`;
- пароль по умолчанию: `graphrag-password`;
- database: `neo4j`.

Пароль можно изменить через `GRAPHRAG_NEO4J_PASSWORD` при локальном запуске или
`NEO4J_PASSWORD` в Docker Compose. Для публичного окружения пароль по умолчанию использовать нельзя.
...
### Что находится в демо-корпусе

Все 50 резюме вымышлены, имеют уникальные имена и не содержат контактов реальных людей:

- 5 ролей: Senior Python Backend, Platform, Data, ML и Security Engineer — по 10 профилей;
- 5 городов: Москва, Санкт-Петербург, Казань, Екатеринбург и Новосибирск — по 10 профилей;
- каждая роль представлена в каждом городе двумя кандидатами;
- возраст от 22 до 39 лет, опыт от 3 до 13 лет;
- компании: МТС, Яндекс, Ozon, VK, Альфа-Банк, Авито и Т-Банк;
- у каждого кандидата две записи об опыте работы, проект, достижения и текст «О себе»;
- навыки и технологии включают Python, SQL, Kafka, FastAPI, PostgreSQL, Kubernetes, Docker,
  Terraform, Linux, GCP, Spark, Airflow, ClickHouse, PyTorch, MLOps, MLflow, AWS и SIEM;
- домены: fintech, cloud, retail, healthtech и cybersecurity;
- личные качества в описаниях: коммуникация, ответственность, аккуратность в процессах,
  самостоятельность, доброжелательность и ориентация на результат.

### Рекомендуемые запросы для проверки

Сначала добавьте демо-данные и дождитесь завершения ingestion job. Затем попробуйте:

- `Python-разработчик из Москвы старше 25 лет` — роль, город, навык и возраст;
- `ML-инженер из Санкт-Петербурга с MLOps и Kubernetes` — смешанный русский/английский стек;
- `разработчик, работавший в компании ВК` — алиас кириллицей для `VK`;
- `сотрудник из Москвы, раньше работал в МТС, ответственный и коммуникативный` — строгие
  метаданные плюс семантические качества;
- `ищу специалистов по данным из Казани со Spark и Airflow` — роль и технологии;
- `масква питон разработчик` — опечатка и разговорное название технологии;
- `Сыщи мужей опытных, кудесников в деле разумов искусственных, обитающих во граде Московском.`
  — свободная стилистика, AI-семантика и архаичная форма города;
- `разработчик, работавший в компании Microsoft` и `сотрудник из несуществующего города` —
  демонстрация описанного ниже ограничения неизвестных сущностей.
